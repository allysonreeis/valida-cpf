// 705.484.450-52 070.987.720-03
/*
1° digito: Usando os primeiros 9 digitos do CPF

7x  0x 5x 4x 8x 4x 4x 5x 0x 
10  9  8  7  6  5  4  3  2
70  0  40 28 48 20 16 15 0 = 237

total de digitos - (somatório * total de dígitos) =  1° dígito CPF
11 - (237 % 11) = 5 (Primeiro dígito)

Se o número digito for maior que 9, consideramos 0.

7x  0x 5x 4x 8x 4x 4x 5x 0x 5x
11 10  9  8  7  6  5  4  3  2
77  0  45 32 56 24 20 20 0  10 = 284

11 - (284 % 11) = 2 (Segundo dígito)
*/

function validaCPF (cpf) {
  const cpfLimpo = cpf.replace(/\D+/g, '');
  let cpfValido = Array.from(cpfLimpo.slice(0,-2));
  let contadorDez = 10;
  let contadorOnze = 11;

  let primeiroDigito = cpfValido.reduce((acumulador, num)=> {
    acumulador += Number(num)*contadorDez;
    contadorDez--;

    return acumulador;
  }, 0);

  primeiroDigito = 11 - (primeiroDigito % 11);
  if (primeiroDigito > 9) primeiroDigito = 0;

  cpfValido.push(primeiroDigito.toString());
  
  let segundoDigito = cpfValido.reduce((acumulador, num)=> {

    acumulador += Number(num)*contadorOnze;
    contadorOnze--;

    return acumulador;
  }, 0);

  segundoDigito = 11 - (segundoDigito % 11);

  if (segundoDigito > 9) segundoDigito = 0;

  cpfValido.push(segundoDigito.toString());
  
  if (cpfLimpo === cpfValido.join('')) {
    console.log( `O cpf: ${cpfValido.join('').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, `$1.$2.$3-$4`)} é válido!`);
  }
}

validaCPF.prototype.criaDigito = function (){};
validaCPF.prototype.verificaCpfDigitado = function() {};


//validaCPF('705.484.450-52');
validaCPF('070.987.720-03');
